...
if main_menu == 4 && max(track) >= 3
    ...
    %Begin inversion
    X2 = []; rms = []; R1= []; D1 = [];
    res = W*(F(1,:)'-d'); %the normalized residual
    
    W_orig = W;
    
    fac = 4;
    
    indres = abs(res)-rms_goal/fac<10^-10;
    
    v(1,:) = diag(W);
    v(1,indres) = (1/fac)*1./(F(1,indres)'-d(indres)');
    W = diag(v(1,:));
    
    res(:,1) = W*(F(1,:)'-d'); %the normalized residual
    
    rms(1) = norm(res(:,1))/sqrt(2*nd); %Initial rms of starting model
    ...
    for iter=2:itmax
        ...
        indres = abs(res(:,iter))-rms_goal/fac<10^-10;

        v(iter,:) = diag(W);
        v(iter,indres) = (1/fac)*1./(F(iter,indres)'-d(indres)');
        W = diag(v(iter,:));
