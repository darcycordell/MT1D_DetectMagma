...
% fid = fopen(['geometry_',num2str(geometrynum),'_err_02.txt']);
% load(['geometry_',num2str(geometrynum),'_err_02_run01.mat']);
% load(['geometry_',num2str(geometrynum),'_err_02_run01_MCMCoutput.mat']);
...
if geometrynum == 3 || geometrynum == 4
    nLayer = 3;
else
    nLayer = 2;
end

freq_array = MTdata.freq_array;
...
target_rms = 1.2; %Play with this number
...
val1 = mv(log10(rhob(nLayer,ind,burnin:end)));
val2 = mv(10.^(models(2*P.nL-2,ind,burnin:end)));
val3 = mv(log10(10.^(models(2*P.nL-1,ind,burnin:end))-10.^(models(2*P.nL-2,ind,burnin:end))));
val4 = mv((models(P.nL-1,ind,burnin:end)));
...
[mler,mle_plusr,mle_minusr,~,~] = mle_calc(val1,0.1,50);

[mled,mle_plusd,mle_minusd,~,~] = mle_calc(val2,0.1,1);
[mlet,mle_plust,mle_minust,~,~] = mle_calc(val3,0.1,25);

[mlem,mle_plusm,mle_minusm,~,~] = mle_calc(val4,0.1,1);


% mler = mean(val1);
% mle_plusr = mean(val1)+std(val1);
% mle_minusr = mean(val1)-std(val1);
% 
% mled = mean(val2);
% mle_plusd = mean(val2)+std(val2);
% mle_minusd = mean(val2)-std(val2);
% 
% mlet = mean(val3);
% mle_plust = mean(val3)+std(val3);
% mle_minust = mean(val3)-std(val3);


% mlem = median(val4);
% mle_plusm = median(val4)+std(val4);
% mle_minusm = median(val4)-std(val4);
...
if geometrynum == 2 || geometrynum == 4
    true_thick = 200;
else
    true_thick = 1000;
end
...
rhogrid = linspace(minrho,maxrho,200); %Gridded log resistivity
dgrid = linspace(0,10^maxdepth,2000); %Gridded depths (meters)
